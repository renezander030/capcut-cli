...
export function findDraft(input: string): string {
  ...
  // If it's a directory, look for draft files inside
  const candidates = [
...
export function updateTextContent(content: string, newText: string): string {
  try {
    ...
    if (parsed.text !== undefined) {
      parsed.text = newText;
      // Update style ranges if present
    ...
    // XML-like format: replace text between innermost tags
    const match = content.match(/^(.*\])?(.*?)(\[.*)?$/s);
    if (match) {
      // Find the text portion inside [brackets] in legacy format
