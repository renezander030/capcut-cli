...
> **New in v0.19.0:** nine items from an opportunity-mining pass over this repo's issues and the surrounding ecosystem, in two clusters. **ffmpeg robustness:** a long `render` no longer dies with `ERR_CHILD_PROCESS_STDIO_MAXBUFFER` on a draft that was fine — that spawn was the last one without a `maxBuffer`, and the failure was reported as "install ffmpeg" even though ffmpeg had already rendered most of the file; `render --progress` streams ffmpeg's output so a ten-minute job stops looking hung; and a failed render now names the missing decoder, encoder or filter instead of dumping stderr. **Caption quality:** `lint` gained reading-speed (`caption-too-fast`), vertical safe-area (`caption-outside-safe-area`) and speed-consistency checks, and `lint --fix` can finally re-wrap CJK captions — it inserts a break between characters and shifts the `styles[]` UTF-16LE byte ranges across it, so karaoke highlights stay on their characters. Plus `export-timeline` no longer rounds a sub-half-frame clip to a zero-length OTIO clip ([#82](https://github.com/renezander030/capcut-cli/issues/82)), and the keyframe docs now name the `property_type` the code actually writes ([#80](https://github.com/renezander030/capcut-cli/issues/80)). Full details in the [changelog](./CHANGELOG.md).

> **New in v0.18.0:** `capcut fixture` now redacts the device identifiers CapCut stamps into every draft, so a bundle is finally safe to attach to a public issue (#59); `init` warns when the bundled template predates the CapCut version that owns the target drafts folder, instead of letting the app refuse the draft with a misleading "unusual path" error (#67); the nested `Timelines/` guidance is gated on detected app version, so 8.5.0 users stop seeing a 7.x warning their own evidence contradicts (#68); and the schema docs record CapCut's frame quantisation of durations on first open (#69). Full details in the [changelog](./CHANGELOG.md).
